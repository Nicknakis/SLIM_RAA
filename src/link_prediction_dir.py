...
class LP_():
    def __init__(self,latent_z,latent_w,gamma_1,gamma_2,delta_1,delta_2,dataset,sparse_i,sparse_j,weights_signed,device):
        ...
        removed_edges=np.loadtxt("./datasets/directed/"+dataset+"/removed_edges.txt")
        ...
        self.sparse_i_zer=np.loadtxt("./datasets/directed/"+dataset+'/sparse_i_zer.txt')
        self.sparse_j_zer=np.loadtxt("./datasets/directed/"+dataset+'/sparse_j_zer.txt')
        ...
        self.train_zer_i=np.loadtxt("./datasets/directed/"+dataset+'/sparse_i_zer_train.txt').astype(int)
        self.train_zer_j=np.loadtxt("./datasets/directed/"+dataset+'/sparse_j_zer_train.txt').astype(int)
    ...
    def vec1(self,i,j):
        ...
        d=-((((self.latent_z[i]-self.latent_w[j])**2).sum(-1))**0.5)+self.gamma_1[i]+self.gamma_2[j]
        return np.exp(d.detach().cpu().numpy())    
    
    def vec2(self,i,j):
        d=((((self.latent_z[i]-self.latent_w[j])**2).sum(-1))**0.5)-self.delta_1[i]-self.delta_2[j]
        return np.exp(d.detach().cpu().numpy())    
